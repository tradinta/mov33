...
import { Bell, User, LogOut, Search, Command, ChevronDown, Sparkles } from 'lucide-react';
...
function Clock() {
    ...
    return (
        <div className="hidden md:flex flex-col items-end">
            <span className="text-xl font-headline font-black text-foreground tabular-nums leading-none">{time}</span>
            <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground mt-1">Local Time</span>
        </div>
    );
}
...
export function AdminTopbar() {
    ...
    return (
        <header className="sticky top-0 z-40 h-20 w-full border-b border-border/50 bg-background/80 backdrop-blur-xl supports-[backdrop-filter]:bg-background/60 flex items-center justify-between px-8">
            {/* Left: Search Bar */}
                    ...
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" className="relative flex items-center gap-3 h-12 pr-2 pl-2 rounded-2xl hover:bg-muted/50 transition-all duration-300 select-none">
                            ...
                            <div className="hidden lg:flex flex-col items-start mr-2">
                                <span className="text-sm font-black uppercase italic tracking-tighnt leading-none text-foreground">
                                    {profile?.displayName?.split(' ')[0] || 'Admin'}
                                </span>
                                <span className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest mt-1">Staff Access</span>
                            </div>
                        ...
                        <DropdownMenuLabel className="p-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex flex-col">
                                    ...
                                    <p className="text-[10px] text-muted-foreground/60 font-medium tracking-tight mt-1">{profile?.email}</p>
                                </div>
                            </div>
                        </DropdownMenuLabel>
